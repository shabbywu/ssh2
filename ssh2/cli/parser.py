...
class YamlParser:
    ...
    def _parse(self, document, session=None):
        ...
        cls = KindMapper[kind]
        return self.__parse(cls, spec, filter_by=obj_filter_by, filter_value=obj_filter_value, session=session)
    ...
    def parse_auth_method(cls, spec: dict, instance: AuthMethod = None):
        ...
        if type == AuthMethodType.PASSWORD:
            ...
            new_instance = AuthMethod.from_publishkey_file(content, save_private_key_in_db=save_private_key_in_db, name=name)
    ...
    def parse_session(cls, spec: dict, instance: Session = None):
        ...
        return Session(name=name, tag=tag, client=client, server=server,
